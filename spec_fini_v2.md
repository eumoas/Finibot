...
**Versão:** 2.0 | **Data:** 2026-05-23 | **Revisão de:** spec v1.0
...
|---|---|---|
| RF-01 | O bot DEVE detectar se o usuário é novo (sem cadastro no banco) e iniciar o onboarding automaticamente ao primeiro `/start` | 🔴 Must |
| RF-02 | O onboarding DEVE coletar: nome, idade e **fonte de renda principal** (mesada / estágio / freelas / trabalho formal / outras) via InlineKeyboard | 🔴 Must |
| RF-03 | O onboarding DEVE coletar o valor aproximado de renda mensal via texto livre — com instrução clara de que pode ser fictício | 🔴 Must |
| RF-04 | O onboarding DEVE aplicar quiz diagnóstico de 3 perguntas via InlineKeyboard | 🔴 Must |
| RF-05 | As respostas do quiz DEVEM categorizar o usuário em perfil: **Iniciante / Em Desenvolvimento / Avançado** — e ajustar o tom das mensagens subsequentes | 🔴 Must |
| RF-06 | Ao concluir o onboarding, o bot DEVE: (a) mostrar o perfil diagnosticado, (b) sugerir imediatamente o primeiro registro de gasto como próximo passo, (c) conceder +100 pts | 🔴 Must |
| RF-07 | O usuário DEVE poder refazer o onboarding com `/recomecar` (apaga histórico e pontos) | 🟢 Could |
...
| ID | Requisito | Prioridade |
|---|---|---|
| RF-08 | O bot DEVE responder perguntas sobre finanças pessoais em linguagem acessível para jovens | 🔴 Must |
| RF-09 | O bot DEVE cobrir no mínimo 50 tópicos em 6 categorias (ver mapa abaixo) | 🔴 Must |
| RF-10 | Respostas DEVEM ter no máximo 150 palavras | 🔴 Must |
| RF-11 | O bot DEVE usar exemplos contextualizados para a faixa etária (mesada, lanche, streaming, transporte escolar) | 🔴 Must |
| RF-12 | O bot DEVE terminar respostas do Q&A com pergunta reflexiva ou mini-desafio conectado ao contexto financeiro atual do usuário | 🟡 Should |
| RF-13 | O bot NÃO DEVE recomendar ações, FIIs, criptomoedas ou produtos específicos | 🔴 Must |
| RF-14 | Se não souber responder, o bot DEVE admitir e sugerir fontes confiáveis (Banco Central, ENEF, Consumidor.gov.br) | 🔴 Must |
| RF-15 | O Q&A DEVE usar o contexto financeiro do usuário para personalizar a resposta quando relevante (ex: "você gastou R$X em lazer esse mês, então...") | 🟡 Should |
...
| ID | Requisito | Prioridade |
|---|---|---|
| RF-16 | O comando `/simular` DEVE iniciar o simulador interativo | 🔴 Must |
| RF-17 | O simulador DEVE aceitar parâmetros em linguagem natural ("se eu guardar R$50 por mês por 1 ano") | 🔴 Must |
| RF-18 | O simulador DEVE mostrar três cenários comparativos: **gastar agora** / **guardar na poupança** / **investir no Tesouro Selic** | 🔴 Must |
| RF-19 | O simulador DEVE usar taxas reais aproximadas (Selic atual, poupança atual) com nota de transparência sobre a fonte e data | 🟡 Should |
| RF-20 | O resultado DEVE ser apresentado como tabela de texto formatada no Telegram (sem imagem) | 🟡 Should |
| RF-21 | O simulador DEVE conectar o resultado com uma meta ativa do usuário, se houver ("isso pagaria X% da sua meta 'Celular novo'") | 🟡 Should |
...
| ID | Requisito | Prioridade |
|---|---|---|
| RF-22 | O comando `/desafio` DEVE exibir o desafio da semana atual | 🔴 Must |
| RF-23 | O banco de desafios DEVE conter no mínimo 20 desafios variados | 🔴 Must |
| RF-24 | Desafios DEVEM ter dificuldade categorizada: fácil (50pts) / médio (100pts) / difícil (150pts) | 🟡 Should |
| RF-25 | O usuário DEVE poder marcar um desafio como concluído via InlineKeyboard | 🔴 Must |
| RF-26 | Desafios DEVEM estar conectados ao módulo de controle financeiro sempre que possível (ex: "registre todos os gastos de hoje") | 🔴 Must |
| RF-27 | O bot DEVE enviar lembrete opt-in do desafio (uma vez por semana via scheduled job) | 🟢 Could |
...
| ID | Requisito | Prioridade |
|---|---|---|
| RF-28 | O sistema DEVE atribuir pontos a todas as ações relevantes do usuário | 🔴 Must |
| RF-29 | O sistema DEVE ter 5 níveis com nomes temáticos e thresholds claros | 🔴 Must |
| RF-30 | O bot DEVE notificar o usuário ao subir de nível com mensagem comemorativa e sugestão do que fazer em seguida | 🔴 Must |
| RF-31 | O comando `/pontos` DEVE exibir: pontuação atual, nível, quanto falta para o próximo e um resumo das últimas ações pontuadas | 🔴 Must |
| RF-32 | O bot DEVE exibir o nível do usuário em todas as mensagens de progresso | 🟡 Should |
| RF-33 | O sistema de pontos DEVE dar peso maior a ações de registro financeiro do que a perguntas ao Q&A | 🔴 Must |
...
7 dias consecutivos com ≥1 lançamento   │   +80  │ F9 streak
...
| ID | Requisito | Prioridade |
|---|---|---|
| RF-34 | O comando `/meta` DEVE iniciar fluxo conversacional para criar meta com: título, valor-alvo e prazo | 🔴 Must |
| RF-35 | O bot DEVE calcular e mostrar automaticamente: valor mensal necessário para atingir a meta no prazo | 🔴 Must |
| RF-36 | O bot DEVE alertar se o valor mensal necessário for maior que a renda informada no onboarding | 🔴 Must |
| RF-37 | O usuário DEVE poder atualizar o valor atual de uma meta com `/meta atualizar` ou frase em linguagem natural | 🔴 Must |
| RF-38 | O bot DEVE armazenar até 5 metas ativas por usuário no MVP | 🟡 Should |
| RF-39 | Ao atingir 100%, o bot DEVE enviar mensagem comemorativa personalizada + +80 pts | 🟡 Should |
| RF-40 | O comando `/metas` DEVE listar todas as metas ativas com barra de progresso em texto ASCII e dias restantes | 🔴 Must |
| RF-41 | O simulador (F3) DEVE referenciar as metas ativas ao exibir resultados | 🟡 Should |
...
| ID | Requisito | Prioridade |
|---|---|---|
| RF-42 | No 1º dia de cada mês, o bot DEVE enviar ao usuário um resumo do mês anterior (via scheduled job) | 🟡 Should |
| RF-43 | O relatório DEVE incluir: total de receitas, total de despesas, saldo, breakdown por categoria, pontos ganhos, desafios completados, metas ativas e evolução de nível | 🟡 Should |
| RF-44 | O relatório DEVE incluir um **insight educativo contextualizado**: a categoria com maior gasto recebe um comentário específico (ex: "Você gastou R$X em lazer — isso é Y% da sua renda. A recomendação do método 50-30-20 é no máximo 30%.") | 🔴 Must |
| RF-45 | O relatório DEVE incluir a planilha `.xlsx` como anexo | 🟡 Should |
| RF-46 | O usuário DEVE poder opt-out do relatório mensal com `/silenciar relatorio` | 🟡 Should |
| RF-47 | O usuário DEVE poder solicitar o relatório do mês atual a qualquer hora com `/resumo mes` | 🔴 Must |
...
| ID | Requisito | Prioridade |
|---|---|---|
| RF-48 | O comando `/ajuda` DEVE exibir menu com todos os comandos, organizados por contexto de uso | 🔴 Must |
| RF-49 | O menu DEVE usar InlineKeyboard para navegação entre categorias | 🟡 Should |
...
| ID | Requisito | Prioridade |
|---|---|---|
| RF-50 | O usuário DEVE poder registrar receitas e despesas por mensagem em linguagem natural sem comandos | 🔴 Must |
| RF-51 | O bot DEVE identificar automaticamente: valor, tipo (receita/despesa), categoria, data e descrição | 🔴 Must |
| RF-52 | O bot DEVE pedir confirmação antes de salvar um lançamento com InlineKeyboard (Confirmar / Corrigir / Cancelar) | 🔴 Must |
| RF-53 | O usuário DEVE poder corrigir valor, categoria, data ou descrição antes de confirmar | 🔴 Must |
| RF-54 | Após confirmar o lançamento, o bot DEVE exibir o saldo do mês atualizado e um insight rápido quando relevante | 🔴 Must |
| RF-55 | O bot DEVE reconhecer expressões de tempo relativas: "hoje", "ontem", "segunda-feira", "semana passada" | 🔴 Must |
...
RECEITAS (5 categorias):
  ...
  Estágio        → salário de estágio ou jovem aprendiz
  Freelas        → trabalhos avulsos, bicos, serviços
  Presente       → dinheiro recebido de presente
...
| ID | Requisito | Prioridade |
|---|---|---|
| RF-56 | O comando `/resumo` DEVE exibir: total de receitas, total de despesas, saldo, e breakdown por categoria (valor + % da renda) | 🔴 Must |
| RF-57 | O resumo DEVE destacar visualmente a categoria com maior gasto | 🔴 Must |
| RF-58 | O resumo DEVE incluir comparação com o mês anterior quando houver dados | 🟡 Should |
| RF-59 | O resumo DEVE incluir um insight do Fini contextualizado com os dados reais do usuário | 🔴 Must |
| RF-60 | O resumo DEVE mostrar o progresso das metas ativas | 🟡 Should |
...
| ID | Requisito | Prioridade |
|---|---|---|
| RF-61 | O comando `/planilha` DEVE gerar e enviar arquivo `.xlsx` diretamente no chat do Telegram | 🔴 Must |
| RF-62 | A planilha DEVE conter duas abas: **Lançamentos** e **Resumo** | 🔴 Must |
| RF-63 | A aba Lançamentos DEVE ter colunas: Data, Tipo, Categoria, Descrição, Valor | 🔴 Must |
| RF-64 | A aba Resumo DEVE ter: total de receitas, total de despesas, saldo, tabela de gastos por categoria com valor e % e gráfico de barras (se suportado pelo openpyxl) | 🔴 Must |
| RF-65 | A planilha DEVE ter cabeçalhos com visual jovem: cores verde (receita) / vermelho (despesa) / azul (neutro), fonte Arial 11 | 🟡 Should |
| RF-66 | A planilha DEVE ter filtros automáticos na aba Lançamentos | 🟡 Should |
| RF-67 | O bot DEVE descartar o arquivo `.xlsx` do servidor após o envio — armazena apenas os dados estruturados no banco | 🔴 Must |
| RF-68 | O usuário DEVE receber +20 pts ao exportar a planilha pela primeira vez no mês | 🟡 Should |
...
| ID | Requisito | Prioridade |
|---|---|---|
| RF-69 | O usuário DEVE poder enviar foto de nota fiscal ou cupom para extração automática via visão do LLM | 🟡 Should |
| RF-70 | O bot DEVE inferir valor total, estabelecimento e data da nota | 🟡 Should |
| RF-71 | O bot DEVE seguir o mesmo fluxo de confirmação (RF-52) após extrair os dados | 🔴 Must |
| RF-72 | O bot DEVE descartar a imagem imediatamente após o processamento — nunca armazenar a foto | 🔴 Must |
...
| ID | Requisito | Prioridade |
|---|---|---|
| RF-73 | O comando `/aprender` DEVE abrir um menu de tópicos educativos de educação financeira | 🔴 Must |
| RF-74 | O módulo DEVE conter, no MVP, os tópicos: receita/despesa, saldo, porcentagem, juros simples, juros compostos, metas, planejamento, consumo consciente e bets/apostas | 🔴 Must |
| RF-75 | Cada tópico DEVE exibir um card curto com conceito, exemplo em reais, quiz e mini-desafio prático | 🔴 Must |
| RF-76 | Os quizzes DEVEM usar InlineKeyboard e retornar feedback imediato para resposta certa ou errada | 🔴 Must |
| RF-77 | Os mini-desafios DEVEM conectar o aprendizado aos comandos do bot quando possível (`/gasto`, `/receita`, `/resumo`, `/meta`, `/simular`) | 🟡 Should |
| RF-78 | O conteúdo DEVE usar dados reais do mês quando disponíveis, especialmente em saldo, porcentagem e planejamento | 🟡 Should |
| RF-79 | O tópico bets/apostas DEVE tratar apostas como educação financeira de risco, sem incentivo e com orientação de proteção financeira | 🔴 Must |
| RF-80 | A interação com o módulo Aprender DEVE pontuar ações educativas: visualizar tópico, acertar quiz e aceitar mini-desafio | 🟡 Should |
...
- Análise de risco em apostas/bets como tema de cidadania financeira.
...
| RN-03 | Streak de 7 dias: precisa de ≥1 lançamento por dia. Interrupção zera o streak |
...
*Fini — Seu Parceiro Financeiro | spec.md v2.0 | 2026-05-23*
*Revisão de: spec v1.0 (2026-04-04)*
